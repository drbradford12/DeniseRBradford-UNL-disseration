...
We would like to test the combination with interactive features, such as map selection and highlighted lines in Parallel Coordinate Plot, for example.\svp{You don't have to motivate your other papers here - that will come later. Focus on the essentials - describe the main pieces of work in the area you're trying to expound upon.}
...
Tukey was a person who helped utilize the likes of stem-and-leaf plot, boxplot-resistant smooth, and rootgram.
...
The grammar of graphics (gg of ggplot2) is a theory that is well-defined for creating statistical graphics with work from Wilkinson [@Wilkinson1999] and Hadley Wickham [@ggplot2]. 
...
Seven major components help build effective visualizations that build on one another. 
Seven major components help build effective visualizations. 
The following diagram will display the effectiveness of data graphics:
...
\svp{You don't need this, you need to actually demonstrate the grammar of graphics with actual data and code :). See my dissertation for an example, but find your own dataset to use. My focus was on grammar of graphics and how it makes testing graphics easy, but yours will be a bit different. You may want to compare to a non-grammar approach in terms of the amount of code required and scale defaults and such - we can talk about that.}
